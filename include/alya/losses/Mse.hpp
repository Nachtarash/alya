...
/// @brief Mean Squared Error Loss | Measures the average squared difference between prediction and target
/// @tparam P Precison type (e.g. alya::fp32)
/// @note Formula: L = (1/n) Σ (x_i - y_i)^2
template <typename P>
class mseLoss : public internal::Loss<P> {
public:
...
private:
    internal::lossResult<P> forwardCpu(const Tensor<P, 2>& logits, const Tensor<P, 2>& targets) {
        ...
        for(size_t i = 0; i < batch * cols; i++) {
            ...
            computeT GVal = computeT(2) * diff / (batch * cols);
